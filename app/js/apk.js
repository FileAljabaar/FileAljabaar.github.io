...
http.open('get', 'app/json/apk.json', true);
...
http.onload = function(){
	...
	if(this.readyState == 4 && this.status == 200){
		...
		for(let item of products){
			output += `
 <div class="col-sm-6 hide col-md-6 col-lg-6 m-0 p-0 bg-light shadow-0 product-hover mt-1" style="max-width: 45em">
          ...
          <div class="card-body bg-light p-1">
            <div class="d-flex justify-content-between p-md-1">
              <div class="d-flex flex-row">
                ...
                <div>
                  <h7 class="text-danger fw-bold">${item.title}</h7>
                  <p class="mb-0 fs-8 fs-sm text-dark">${item.descriptions}</p>
                  <span class="badge rounded-pill badge-danger fs-sm">${item.badge}</span>
                </div>
              </div>
            </div>
          </div>
